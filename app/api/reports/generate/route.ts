...
function buildPrompt(topic: string, context: string, today: string): string {
  return `분석 주제: ${topic}
작성일: ${today}
...
export async function POST(req: NextRequest) {
  ...
  console.log(`[report-gen] 주제: "${topic}", 테이블: ${[...tables].join(', ')}`);
  ...
  try {
    const msg = await anthropic.messages.create({
      ...
      max_tokens: 8000,
      system:     SYSTEM,
      messages: [{ role: 'user', content: buildPrompt(topic, context, today) }],
    });
  ...
  const safeTitle  = topic.slice(0, 40).replace(/[^\w가-힣]/g, '_');
