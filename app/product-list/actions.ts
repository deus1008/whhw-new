...
/** 제품 생동/DMF/대조약 여부 수동 수정 (관리자 전용). value: true/false/null(미확인) */
export async function updateProductFlag(
  id: string,
  field: 'is_bioequiv' | 'has_dmf' | 'is_reference_drug',
  value: boolean | null,
): Promise<{ error?: string }> {
  const ALLOWED = ['is_bioequiv', 'has_dmf', 'is_reference_drug'];
  if (!ALLOWED.includes(field)) return { error: '잘못된 필드입니다.' };
