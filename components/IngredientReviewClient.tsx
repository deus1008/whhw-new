  ...
  return (
    <div style={{ marginTop: '1.25rem', display: 'flex', flexDirection: 'column', gap: '0.9rem' }}>
      <div style={{
        ...
        설명은 <b style={{ color: '#b45309' }}>식약처 허가 효능효과</b>를 근거로 AI가 요약한 것입니다.
        영업 현장에 나가는 내용이므로 검수 후 <b style={{ color: '#b45309' }}>검수완료</b>로 표시해주세요.
        <span style={{ color: '#b45309' }}> 허가사항 미연동</span> 항목은 허가 원문 없이
        ...
        {TABS.map(t => (
          <button key={t.key} onClick={() => setTab(t.key)} title={t.hint}
            style={{
              ...
              borderColor: tab === t.key ? 'rgba(147,197,253,0.4)' : '#e5e9f0',
              background: tab === t.key ? 'rgba(147,197,253,0.12)' : '#ffffff',
      ...
      {shown.length === 0 ? (
        <div style={{
          ...
          border: '1px dashed #e5e9f0', borderRadius: '12px', fontSize: '0.85rem',
        }}>
          해당 항목이 없습니다.
        </div>
      ) : (
...
function Card({ item, onPatch }: { item: ReviewItem; onPatch: (i: string, p: Partial<ReviewItem>) => void }) {
  ...
  return (
    <div style={{
      background: '#ffffff',
      border: `1px solid ${item.reviewed ? 'rgba(110,231,183,0.2)' : '#e5e9f0'}`,
      borderRadius: '12px', padding: '0.8rem 1rem',
    }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap', marginBottom: '0.5rem' }}>
        <span style={{ fontSize: '0.85rem', fontWeight: 700, color: '#2563eb' }}>{item.ingredient}</span>
        {item.drugClass && (
          <span style={{
        ...
        {item.context && (
          <span style={{ fontSize: '0.65rem', color: '#94a3b8' }}>{item.context}</span>
        )}
        <span style={{ marginLeft: 'auto', display: 'flex', gap: '6px' }}>
          {!editing && (
            <button onClick={() => setEditing(true)} disabled={busy} style={btn('#2563eb')}>수정</button>
          )}
          <button onClick={toggleReviewed} disabled={busy}
            style={btn(item.reviewed ? '#059669' : '#94a3b8')}>
      ...
      {editing ? (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
          ...
          <div style={{ display: 'flex', gap: '6px' }}>
            <button onClick={save} disabled={busy} style={btn('#059669')}>
              {busy ? '저장 중…' : '저장하고 검수완료'}
            </button>
            <button onClick={() => { setDesc(item.description); setCls(item.drugClass); setEditing(false); }}
              disabled={busy} style={btn('#94a3b8')}>취소</button>
          </div>
        </div>
      ) : (
        <div style={{ fontSize: '0.78rem', lineHeight: 1.65, color: '#475569' }}>
          {item.description}
        </div>
      )}
      {err && <div style={{ marginTop: 6, fontSize: '0.72rem', color: '#dc2626' }}>{err}</div>}
    </div>
  );
}
...
function btn(color: string): React.CSSProperties {
  return {
    ...
    fontFamily: 'inherit', background: '#f1f5f9',
    border: '1px solid #e5e9f0', color,
  };
}
function input(): React.CSSProperties {
  return {
    width: '100%', padding: '0.45rem 0.7rem', background: '#ffffff',
    border: '1px solid #d7dce5', borderRadius: '8px',
    color: '#111827', fontSize: '0.78rem', outline: 'none', fontFamily: 'inherit',
  };
}
