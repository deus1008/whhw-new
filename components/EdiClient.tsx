...
/*  비교 보기 — 섹션별 월별 나란히 비교                        */
/* ════════════════════════════════════════════════════════════ */
type BasicStat = { name: string; amount: number };
    ...
    <Section title={title}>
      <div style={{ overflowX: 'auto' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.8rem' }}>
          <thead>
            <tr>
              <th rowSpan={2} style={{ ...TH('left'), width: 120, minWidth: 120, verticalAlign: 'middle', textAlign: 'center' }}>이름</th>
              {periods.map((p, pi) => (
                <th key={pi} style={{
            ...
            <tr>
              {periods.map((_, pi) => (
                <th key={pi} style={{ ...TH('right'), ...BL, fontSize: '0.7rem' }}>처방액</th>
              ))}
            </tr>
          </thead>
          <tbody>
            ...
            <tr style={TR_TOTAL}>
              ...
              {reports.map((r, fi) => {
                ...
                return (
                  <td key={fi} style={{ ...TD('right', true), ...BL, fontSize: '0.78rem' }}>{fmt(totAmt)}</td>
                );
              })}
              {nFiles >= 2 && (() => {
...
function EdiDashboard({ data }: { data: EdiData }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.2rem' }}>
      {hasSP  && <SalesPersonAccordion stats={data.salesPersonStats} totalAmount={data.totalAmount} />}
      {hasCso && <CsoAccordion         stats={data.csoStats}         totalAmount={data.totalAmount} />}
      {hasHos && <HospitalAccordion    stats={data.hospitalRanking}  totalAmount={data.totalAmount} />}
      {hasItem && (
        <ItemSection
      ...
      {hasPrice && <DrugPriceTable prices={data.drugPrices} />}
...
function SalesPersonAccordion({ stats, totalAmount }: {
  ...
  function toggleCso(spName: string, csoName: string) {
    ...
    setExpandedCsos(prev => { const n = new Set(prev); n.has(key) ? n.delete(key) : n.add(key); return n; });
  }
...
/*  ② CSO별 현황                                              */
/* ════════════════════════════════════════════════════════════ */
function CsoAccordion({ stats, totalAmount }: {
  stats: CsoStat[];
  totalAmount: number;
}) {
  ...
  function toggleCso(name: string) {
    setExpandedCso(prev => { const n = new Set(prev); n.has(name) ? n.delete(name) : n.add(name); return n; });
  }
  function toggleHos(csoName: string, hosName: string) {
    const key = `${csoName}||${hosName}`;
    setExpandedHos(prev => { const n = new Set(prev); n.has(key) ? n.delete(key) : n.add(key); return n; });
  }
...
/*  ③ 처방처별 현황                                            */
/* ════════════════════════════════════════════════════════════ */
function HospitalAccordion({ stats, totalAmount }: {
  stats: HospitalStat[];
  totalAmount: number;
}) {
  const [expandedHos,   setExpandedHos]   = useState<Set<string>>(new Set());
  ...
  function toggleHos(name: string) {
    setExpandedHos(prev => { const n = new Set(prev); n.has(name) ? n.delete(name) : n.add(name); return n; });
  }
...
/*  ④ 품목별 현황                                              */
/* ════════════════════════════════════════════════════════════ */
function ItemSection({ stats, hospStats, totalAmount }: {
  stats: ItemStat[];
  ...
  return (
    <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 14, padding: '1.1rem 1.2rem' }}>
      ...
      {tab === 0
        ? <ItemCsoAccordion stats={stats} search={search} totalAmount={totalAmount} />
        : <ItemHospAccordion stats={hospStats} search={search} totalAmount={totalAmount} />
      }
    </div>
  );
}
...
/* ── 뷰1: 품목 → CSO → 요양기관 ──────────────────────────── */
function ItemCsoAccordion({ stats, search, totalAmount }: {
  stats: ItemStat[];
  search: string;
  totalAmount: number;
}) {
  ...
  function toggleItem(name: string) {
    setExpandedItems(prev => { const n = new Set(prev); n.has(name) ? n.delete(name) : n.add(name); return n; });
  }
...
/* ── 뷰2: 품목 → 요양기관 → 담당자 → CSO ─────────────────── */
function ItemHospAccordion({ stats, search, totalAmount }: {
  stats: IHItemStat[];
  search: string;
  totalAmount: number;
}) {
