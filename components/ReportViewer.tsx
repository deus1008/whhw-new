...
export default function ReportViewer({ report }: { report: Report }) {
  return (
    <>
      ...
      {/* 제목 */}
      <h1 className="print-title" style={{ fontSize: '1.5rem', fontWeight: 800, color: '#111827', margin: '0 0 0.4rem' }}>
        {report.title}
      </h1>
      <p className="print-meta" style={{ fontSize: '0.75rem', color: '#94a3b8', marginBottom: '2rem' }}>
...
const MD_COMPONENTS = {
  h1: ({ children }: { children?: React.ReactNode }) => (
    <h1 style={{ fontSize: '1.35rem', fontWeight: 800, color: '#111827', margin: '1.8rem 0 0.6rem', borderBottom: '1px solid #e5e9f0', paddingBottom: '0.4rem' }}>{children}</h1>
  ),
  h2: ({ children }: { children?: React.ReactNode }) => (
    <h2 style={{ fontSize: '1.1rem', fontWeight: 700, color: '#111827', margin: '1.5rem 0 0.5rem' }}>{children}</h2>
  ),
  h3: ({ children }: { children?: React.ReactNode }) => (
    <h3 style={{ fontSize: '0.98rem', fontWeight: 700, color: '#334155', margin: '1.2rem 0 0.4rem' }}>{children}</h3>
  ),
  h4: ({ children }: { children?: React.ReactNode }) => (
    <h4 style={{ fontSize: '0.92rem', fontWeight: 600, color: '#475569', margin: '1rem 0 0.3rem' }}>{children}</h4>
  ),
  p: ({ children }: { children?: React.ReactNode }) => (
    <p style={{ margin: '0 0 0.85rem', color: '#334155' }}>{children}</p>
  ),
  ul: ({ children }: { children?: React.ReactNode }) => (
    <ul style={{ margin: '0 0 0.85rem', paddingLeft: '1.4rem', color: '#334155' }}>{children}</ul>
  ),
  ol: ({ children }: { children?: React.ReactNode }) => (
    <ol style={{ margin: '0 0 0.85rem', paddingLeft: '1.4rem', color: '#334155' }}>{children}</ol>
  ),
  ...
  em: ({ children }: { children?: React.ReactNode }) => (
    <em style={{ color: '#7c3aed' }}>{children}</em>
  ),
  blockquote: ({ children }: { children?: React.ReactNode }) => (
    <blockquote style={{
      borderLeft: '3px solid rgba(96,165,250,0.5)', margin: '1rem 0',
      paddingLeft: '1rem', color: '#64748b', fontStyle: 'italic',
    }}>{children}</blockquote>
  ),
  code: ({ inline, children }: { inline?: boolean; children?: React.ReactNode }) =>
    inline ? (
      <code style={{
        ...
        padding: '0.1em 0.4em', fontSize: '0.85em', color: '#2563eb',
      }}>{children}</code>
    ) : (
  ...
  pre: ({ children }: { children?: React.ReactNode }) => (
    <pre style={{
      ...
      fontSize: '0.83rem', margin: '0 0 1rem', color: '#334155',
    }}>{children}</pre>
  ),
  ...
  th: ({ children }: { children?: React.ReactNode }) => (
    <th style={{
      border: '1px solid #e5e9f0', padding: '0.5rem 0.8rem',
      background: '#f1f4f9', color: '#111827', fontWeight: 600,
      textAlign: 'left',
    }}>{children}</th>
  ),
  td: ({ children }: { children?: React.ReactNode }) => (
    <td style={{
      border: '1px solid #e5e9f0', padding: '0.45rem 0.8rem',
      color: '#334155',
    }}>{children}</td>
  ),
  hr: () => (
    <hr style={{ border: 'none', borderTop: '1px solid #e5e9f0', margin: '1.5rem 0' }} />
  ),
};
