...
const STATUS_META: Record<string, { color: string; bg: string; border: string; label: string }> = {
  '접수':  { color: '#b91c1c', bg: 'rgba(239,68,68,0.12)',  border: 'rgba(239,68,68,0.3)',  label: '접수' },
  '처리중': { color: '#b45309', bg: 'rgba(251,191,36,0.12)', border: 'rgba(251,191,36,0.3)', label: '처리중' },
  '완료':  { color: '#15803d', bg: 'rgba(74,222,128,0.12)', border: 'rgba(74,222,128,0.3)', label: '완료' },
};
...
export default function ErrorReportModal({ onClose, onSeen }: { onClose: () => void; onSeen?: () => void }) {
  ...
  return (
    ...
    >
      <div style={{
        background: '#ffffff', border: '1px solid #e5e9f0', borderRadius: '18px',
        width: '100%', maxWidth: '600px', maxHeight: '88vh', display: 'flex', flexDirection: 'column',
        boxShadow: '0 25px 60px rgba(15,23,42,0.10)',
      }}>
        {/* 헤더 */}
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', padding: '1.4rem 1.6rem 1rem', borderBottom: '1px solid #e5e9f0' }}>
          ...
          <button onClick={onClose} style={{ marginLeft: 'auto', padding: '0.3rem 0.5rem', borderRadius: '6px',
            background: 'transparent', border: '1px solid #e5e9f0', color: 'var(--text-muted)', cursor: 'pointer', fontSize: '0.85rem' }}>✕</button>
        </div>
        ...
        <div style={{ overflowY: 'auto', padding: '1.2rem 1.6rem 1.6rem', display: 'flex', flexDirection: 'column', gap: '1.4rem' }}>
          ...
          <div>
            <div style={{ fontSize: '0.78rem', fontWeight: 700, color: 'var(--text-muted)', marginBottom: '0.6rem' }}>
              📋 내 신고 내역 {myReports.length > 0 && <span style={{ color: '#2563eb' }}>({myReports.length})</span>}
            </div>
            {loadingList ? (
              <p style={{ fontSize: '0.82rem', color: 'var(--text-muted)' }}>불러오는 중…</p>
            ) : myReports.length === 0 ? (
              <p style={{ fontSize: '0.82rem', color: '#94a3b8' }}>아직 신고한 내역이 없습니다.</p>
            ) : (
                ...
                {myReports.map(r => {
                  ...
                  return (
                    <div key={r.id} style={{ border: `1px solid ${m.border}`, borderLeft: `3px solid ${m.color}`,
                      borderRadius: '10px', background: '#f8fafc', padding: '0.8rem 0.9rem' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.35rem', flexWrap: 'wrap' }}>
                        ...
                        <span style={{ marginLeft: 'auto', fontSize: '0.7rem', color: '#94a3b8' }}>{fmtDate(r.created_at)}</span>
                      </div>
                      <div style={{ fontSize: '0.8rem', color: '#475569', lineHeight: 1.6, whiteSpace: 'pre-wrap' }}>{r.content}</div>
                      ...
                      {r.admin_comment ? (
                        <div style={{ marginTop: '0.7rem', background: 'rgba(74,222,128,0.06)', border: '1px solid rgba(74,222,128,0.22)',
                          ...
                          <div style={{ fontSize: '0.7rem', fontWeight: 700, color: '#15803d', marginBottom: '0.3rem' }}>✔ 관리자 조치결과</div>
                          <div style={{ fontSize: '0.83rem', color: '#15803d', lineHeight: 1.65, whiteSpace: 'pre-wrap' }}>{r.admin_comment}</div>
                        </div>
                      ) : (
                        <div style={{ marginTop: '0.55rem', fontSize: '0.74rem', color: '#94a3b8' }}>⏳ 관리자 확인 중입니다.</div>
                      )}
                    </div>
                  );
                })}
          ...
          {/* ── 새 오류 신고 ── */}
          <div style={{ borderTop: '1px solid #e5e9f0', paddingTop: '1.2rem' }}>
            <div style={{ fontSize: '0.78rem', fontWeight: 700, color: 'var(--text-muted)', marginBottom: '0.7rem' }}>➕ 새 오류 신고</div>
            {justSent && (
              <p style={{ margin: '0 0 0.8rem', fontSize: '0.8rem', color: '#15803d' }}>✅ 전송 완료 — 관리자가 확인 후 조치하면 위 내역에 표시됩니다.</p>
            )}
            <form ref={formRef} onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '0.9rem' }}>
              ...
              <div>
                <label style={{ display: 'block', fontSize: '0.76rem', fontWeight: 600, color: 'var(--text-muted)', marginBottom: '0.3rem' }}>
                  오류 내용 <span style={{ color: '#dc2626' }}>*</span>
                </label>
              ...
              <div style={{ display: 'flex', gap: '0.6rem', justifyContent: 'flex-end' }}>
                <button type="button" onClick={onClose} style={{
                  ...
                  border: '1px solid #e5e9f0', background: 'transparent', color: 'var(--text-muted)', cursor: 'pointer' }}>닫기</button>
                <button type="submit" disabled={pending} style={{
                  padding: '0.55rem 1.4rem', borderRadius: '8px', fontSize: '0.85rem',
                  border: '1px solid #ef9a9a', background: '#fef2f2',
                  color: '#dc2626', cursor: pending ? 'not-allowed' : 'pointer', fontWeight: 700 }}>
                  {pending ? '전송 중…' : '🐛 신고하기'}
                </button>
              </div>
            </form>
