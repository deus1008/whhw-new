...
/* ── 컬럼 키워드 매핑 ── */
const CODE_KW    = ['거래처코드','코드','요양기관번호','기관코드','기관번호','번호','code'];
const NAME_KW    = ['거래처명','거래처 명','업체명','기관명','병원명','약국명','요양기관명','상호','name'];
...
const MANAGER_KW = ['지역장','담당지역장','담당자','담당','manager','지점장','매니저'];
const CSO_KW     = ['담당cso','cso명','cso','법인명'];
...
export function parseCustomerBuffer(buffer: Buffer, fileName: string): ParseCustomerResult {
  ...
  // 거래처명 컬럼이 있는 첫 행 찾기
  let headerRowIdx = 0;
  const nameNorm = NAME_KW.map(norm);
  for (let ri = 0; ri < Math.min(headerArrays.length, 8); ri++) {
    const cells = (headerArrays[ri] as unknown[]).map(c => norm(String(c ?? '')));
    const hits = cells.filter(c => nameNorm.some(n => c.includes(n) || n.includes(c)) && c.length >= 2).length;
    if (hits >= 1) { headerRowIdx = ri; break; }
  }
