...
const VALID = new Set(['신제품출시', '정책변경', '기업동향', '이슈사항', '현장동향', '기타']);
const BATCH = 25;
...
async function classifyBatch(key: string, items: { title: string; summary?: string | null }[]): Promise<Classified[]> {
  ...
  const prompt =
    ...
    `(1) type — 유형: 신제품출시 / 정책변경 / 기업동향 / 이슈사항 / 현장동향 / 기타 중 하나\n` +
    `   신제품출시=신제품·신약 출시/발매/허가/개발, 정책변경=약가·급여·제도·규제 변화,\n` +
    `   기업동향=실적·매출·재무·투자·인사·지배구조·M&A 등 기업 경영/외형 동향,\n` +
    `   이슈사항=소송·리콜·품절·계약·행정처분 등, 현장동향=영업·유통·학술·행사\n\n` +
