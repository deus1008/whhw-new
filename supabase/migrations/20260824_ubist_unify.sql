-- UBIST 통합: 진료과 데이터를 ubist_data 단일 테이블로 일원화(조합파일=한 행에 종별+진료과).
--   다처방성분 RPC를 ubist_data(specialty)로 전환, 임시 ubist_specialty 이관·정리·폐기.
...
-- 2) 임시 진료과 데이터(ubist_specialty)를 ubist_data로 이관.
...
-- 3) 진료과 데이터가 생긴 달의 옛 '종별-only' 행 제거(이중집계 방지 — 이후 종별+진료과 통합파일 재업로드로 종별 복원).
DELETE FROM public.ubist_data
WHERE specialty IS NULL
  AND period IN (SELECT DISTINCT period FROM public.ubist_data WHERE specialty IS NOT NULL);
